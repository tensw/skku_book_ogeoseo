import type { Notice, BookReview, Classic100, Bundok } from './types'

export const notices: Notice[] = [
  {
    id: 1,
    title: '2026년 1학기 독서프로그램 신청 안내',
    content: '2026년 1학기 독서프로그램 신청이 시작되었습니다. 이번 학기에는 독서 마라톤, 북 멘토링, 독서 에세이 대회 등 다양한 프로그램이 준비되어 있으니, 많은 관심과 참여 부탁드립니다.\n\n신청 기간은 2026년 2월 17일(월)부터 3월 7일(금)까지이며, 오거서 앱 또는 웹사이트를 통해 신청하실 수 있습니다. 프로그램별 정원이 제한되어 있으므로 조기 마감될 수 있습니다.\n\n문의사항은 학술정보관 독서교육팀(031-290-XXXX)으로 연락해 주시기 바랍니다.',
    date: '2026-02-03',
    views: 487,
    author: '독서교육팀',
    attachments: ['2026-1학기_독서프로그램_안내문.pdf', '프로그램_일정표.xlsx'],
    important: true,
  },
  {
    id: 2,
    title: '제15회 독서에세이 공모전 안내',
    content: '성균관대학교 학술정보관에서 제15회 독서에세이 공모전을 개최합니다. 이번 공모전은 "읽고, 생각하고, 나누다"를 주제로 진행되며, 재학생 누구나 참여 가능합니다.\n\n응모 기간은 2026년 3월 2일(월)부터 4월 30일(수)까지이며, 입상작에는 상장 및 상금이 수여됩니다. 대상(1명) 50만원, 최우수상(2명) 30만원, 우수상(5명) 10만원, 장려상(10명) 5만원입니다.\n\n자세한 응모 요강은 첨부파일을 참고하시기 바랍니다.',
    date: '2026-01-27',
    views: 312,
    author: '독서교육팀',
    attachments: ['제15회_독서에세이_공모전_요강.pdf'],
    important: false,
  },
  {
    id: 3,
    title: '성균 고전 100선 2026년 개정판 안내',
    content: '성균 고전 100선이 2026년 개정판으로 새롭게 업데이트되었습니다. 이번 개정에서는 자연과학 분야 도서 5종이 교체되었으며, 각 도서의 해제가 전면 개정되었습니다.\n\n개정된 목록은 오거서 앱 및 학술정보관 홈페이지에서 확인하실 수 있으며, 인쇄본 책자는 학술정보관 1층 안내데스크에서 배부하고 있습니다.\n\n성균 고전 100선은 본교 교양교육의 핵심으로, 재학 기간 동안 다양한 고전을 접하고 깊이 있는 사유를 경험할 수 있도록 마련된 프로그램입니다.',
    date: '2026-01-20',
    views: 398,
    author: '학술정보관',
    attachments: ['성균고전100선_2026_개정목록.pdf'],
    important: true,
  },
  {
    id: 4,
    title: '오거서 앱 업데이트 안내 (v3.2)',
    content: '오거서 앱이 v3.2로 업데이트되었습니다. 이번 업데이트에서는 독서 기록 통계 기능이 강화되었으며, 독서 모임 채팅 기능이 새롭게 추가되었습니다.\n\n주요 변경사항: 독서 달력 뷰 개선, 리뷰 작성 시 사진 첨부 기능 추가, 푸시 알림 설정 세분화, 다크 모드 지원이 포함됩니다.\n\n앱스토어 및 구글 플레이스토어에서 최신 버전으로 업데이트해 주시기 바랍니다.',
    date: '2026-01-15',
    views: 256,
    author: '정보시스템팀',
    attachments: [],
    important: false,
  },
  {
    id: 5,
    title: '2025학년도 2학기 우수 독서인 시상',
    content: '2025학년도 2학기 우수 독서인으로 선정된 학우들을 축하합니다. 이번 학기에는 총 20명의 학우가 우수 독서인으로 선정되었습니다.\n\n선정 기준은 독서 활동 참여도, 서평 작성 품질, 독서 토론 기여도 등을 종합적으로 평가하였습니다. 수상자에게는 상장과 도서 상품권이 수여됩니다.\n\n시상식은 2026년 1월 10일(금) 오후 3시 학술정보관 세미나실에서 진행되었습니다.',
    date: '2025-12-30',
    views: 189,
    author: '독서교육팀',
    attachments: ['2025-2학기_우수독서인_명단.pdf'],
    important: false,
  },
  {
    id: 6,
    title: '겨울방학 독서캠프 참가자 모집',
    content: '2026년 겨울방학 독서캠프 참가자를 모집합니다. 이번 캠프는 "고전과 함께하는 3일"이라는 주제로, 성균 고전 100선 중 엄선된 도서를 집중적으로 읽고 토론하는 프로그램입니다.\n\n일시는 2026년 1월 20일(월)부터 1월 22일(수)까지 2박 3일이며, 장소는 자연과학캠퍼스 수련관입니다. 참가비는 무료이며 숙식이 제공됩니다.\n\n모집 인원은 40명이며, 선착순으로 마감합니다. 신청은 오거서 앱에서 가능합니다.',
    date: '2025-12-20',
    views: 345,
    author: '독서교육팀',
    attachments: ['겨울독서캠프_안내문.pdf', '캠프_일정표.pdf'],
    important: false,
  },
  {
    id: 7,
    title: '도서관 북토크 일정 안내',
    content: '2026년 상반기 도서관 북토크 일정을 안내드립니다. 매월 마지막 주 수요일 오후 4시에 학술정보관 다목적홀에서 진행되며, 각 분야 전문가와 함께 깊이 있는 독서 대화를 나눌 수 있습니다.\n\n2월 북토크는 소설가 김영하 작가를 초청하여 "문학과 기억"을 주제로 진행합니다. 3월에는 철학자 김상환 교수(서울대)가 "동양 고전의 현대적 읽기"를 주제로 강연합니다.\n\n참여를 원하시는 분은 오거서 앱에서 사전 신청해 주시기 바랍니다. 현장 참여도 가능하나 좌석이 제한될 수 있습니다.',
    date: '2025-12-15',
    views: 213,
    author: '학술정보관',
    attachments: ['2026_상반기_북토크_일정.pdf'],
    important: false,
  },
  {
    id: 8,
    title: '독서토론 리더 모집 공고',
    content: '2026년 1학기 독서토론 리더를 모집합니다. 독서토론 리더는 소규모 독서 모임을 이끌며, 참여 학우들의 독서 활동을 촉진하는 역할을 합니다.\n\n지원 자격은 2학기 이상 재학생으로, 독서프로그램 참여 경험이 1회 이상인 학우입니다. 선발된 리더에게는 활동 장학금(학기당 30만원)과 리더십 인증서가 수여됩니다.\n\n지원 기간은 2026년 2월 3일(월)부터 2월 21일(금)까지이며, 오거서 앱 또는 학술정보관 홈페이지에서 지원서를 제출하시면 됩니다.',
    date: '2025-12-10',
    views: 178,
    author: '독서교육팀',
    attachments: ['독서토론리더_모집요강.pdf', '지원서_양식.hwp'],
    important: false,
  },
  {
    id: 9,
    title: '2026년 신간 도서 추천 리스트',
    content: '학술정보관에서 2026년 주목할 만한 신간 도서 추천 리스트를 발표합니다. 각 분야 교수님들의 추천과 학생 투표를 통해 선정된 50권의 도서를 소개합니다.\n\n추천 도서는 문학, 인문, 사회과학, 자연과학, 예술 등 다양한 분야를 아우르며, 학술정보관 1층 "이달의 추천 도서" 코너에서 만나보실 수 있습니다.\n\n전체 추천 리스트는 첨부파일과 오거서 앱에서 확인 가능하며, 대출 예약도 앱을 통해 하실 수 있습니다.',
    date: '2025-12-05',
    views: 267,
    author: '학술정보관',
    attachments: ['2026_신간추천_리스트.pdf'],
    important: false,
  },
  {
    id: 10,
    title: '독서 마일리지 적립 안내',
    content: '독서 마일리지 제도를 안내드립니다. 독서 마일리지는 독서 활동에 참여할 때마다 적립되며, 적립된 마일리지는 다양한 혜택으로 교환할 수 있습니다.\n\n적립 기준: 서평 작성(100P), 독서토론 참여(150P), 독서프로그램 완료(300P), 독서에세이 제출(200P), 북토크 참석(50P)입니다.\n\n마일리지 사용: 5,000P 이상 적립 시 도서 상품권 교환, 10,000P 이상 적립 시 우수 독서인 가산점 부여, 20,000P 이상 적립 시 해외 도서전 참관 우선 선발 혜택이 있습니다.',
    date: '2025-11-28',
    views: 156,
    author: '독서교육팀',
    attachments: ['독서마일리지_안내서.pdf'],
    important: false,
  },
]

export const mockStudents = [
  { id: 1, name: "김서윤", studentId: "2023110001", department: "국어국문학과" },
  { id: 2, name: "이준혁", studentId: "2023120002", department: "경영학과" },
  { id: 3, name: "박지민", studentId: "2024130003", department: "생명과학과" },
  { id: 4, name: "최예린", studentId: "2023140004", department: "철학과" },
  { id: 5, name: "정민수", studentId: "2024150005", department: "컴퓨터공학과" },
  { id: 6, name: "한소희", studentId: "2023160006", department: "사학과" },
  { id: 7, name: "윤태호", studentId: "2024170007", department: "경제학과" },
  { id: 8, name: "강하은", studentId: "2023180008", department: "영어영문학과" },
  { id: 9, name: "서진우", studentId: "2024190009", department: "물리학과" },
  { id: 10, name: "임채원", studentId: "2023200010", department: "미술학과" },
  { id: 11, name: "김도현", studentId: "2024210011", department: "소프트웨어학과" },
  { id: 12, name: "오수빈", studentId: "2023220012", department: "정치외교학과" },
  { id: 13, name: "장민재", studentId: "2024230013", department: "화학과" },
  { id: 14, name: "배서연", studentId: "2023240014", department: "교육학과" },
  { id: 15, name: "홍지아", studentId: "2024250015", department: "의예과" },
]

export const reviews: BookReview[] = [
  {
    id: 1,
    type: 'program',
    user: { name: '김서윤', avatar: 'https://picsum.photos/seed/avatar01/80/80', department: '국어국문학과', studentId: '2023110001' },
    book: { title: '데미안', author: '헤르만 헤세', cover: 'https://picsum.photos/seed/cover-demian/200/280' },
    rating: 5,
    text: '싱클레어의 자아 탐색 여정이 대학생인 지금 읽으니 더욱 깊이 와닿았습니다. "새는 알에서 나오려고 투쟁한다"는 문장이 아직도 마음에 남아있습니다. 독서토론에서 다른 학우들의 해석을 듣고 나니, 같은 책도 이렇게 다르게 읽힐 수 있다는 것이 놀라웠습니다.',
    likes: 72,
    comments: 18,
    timeAgo: '2시간 전',
    category: '문학',
    program: '2025-2학기 독서 마라톤',
    programId: 'classic100',
  },
  {
    id: 2,
    type: 'program',
    user: { name: '이준혁', avatar: 'https://picsum.photos/seed/avatar02/80/80', department: '경영학과', studentId: '2023120002' },
    book: { title: '정의란 무엇인가', author: '마이클 샌델', cover: 'https://picsum.photos/seed/cover-justice/200/280' },
    rating: 4,
    text: '공리주의와 자유주의, 공동체주의의 관점에서 정의를 바라보는 다양한 시각을 접할 수 있었습니다. 특히 트롤리 문제에 대한 토론이 흥미로웠고, 경영학을 공부하는 학생으로서 기업 윤리에 대해 다시 생각해 보는 계기가 되었습니다.',
    likes: 58,
    comments: 23,
    timeAgo: '5시간 전',
    category: '사회과학',
    program: '2025-2학기 북 멘토링',
    programId: 'bundok',
  },
  {
    id: 3,
    type: 'program',
    user: { name: '박지민', avatar: 'https://picsum.photos/seed/avatar03/80/80', department: '생명과학과', studentId: '2024130003' },
    book: { title: '이기적 유전자', author: '리처드 도킨스', cover: 'https://picsum.photos/seed/cover-selfish-gene/200/280' },
    rating: 5,
    text: '유전자의 관점에서 생명 현상을 바라보는 도킨스의 통찰이 정말 인상적이었습니다. 이타적 행동도 결국 유전자의 이기적 전략이라는 설명은 생명과학을 전공하는 저에게도 새로운 시각을 제공했습니다. 밈(meme) 개념의 등장도 흥미로웠습니다.',
    likes: 65,
    comments: 15,
    timeAgo: '1일 전',
    category: '기술과학',
    program: '2025-2학기 독서 마라톤',
    programId: 'classic100',
  },
  {
    id: 4,
    type: 'program',
    user: { name: '최예린', avatar: 'https://picsum.photos/seed/avatar04/80/80', department: '철학과', studentId: '2023140004' },
    book: { title: '존재와 시간', author: '마르틴 하이데거', cover: 'https://picsum.photos/seed/cover-being-time/200/280' },
    rating: 4,
    text: '난해하기로 유명한 하이데거의 저작이지만, 교수님의 북 멘토링 덕분에 핵심 개념들을 이해할 수 있었습니다. 현존재(Dasein)의 개념과 시간성에 대한 분석은 철학 전공자로서 반드시 거쳐야 할 관문이라 느꼈습니다.',
    likes: 41,
    comments: 12,
    timeAgo: '1일 전',
    category: '사회과학',
    program: '2025-2학기 북 멘토링',
    programId: 'bundok',
  },
  {
    id: 5,
    type: 'program',
    user: { name: '정민수', avatar: 'https://picsum.photos/seed/avatar05/80/80', department: '컴퓨터공학과', studentId: '2024150005' },
    book: { title: '코스모스', author: '칼 세이건', cover: 'https://picsum.photos/seed/cover-cosmos/200/280' },
    rating: 5,
    text: '칼 세이건의 우주에 대한 열정과 경외감이 그대로 전해져 오는 명저입니다. 과학적 사실을 시적인 문체로 풀어낸 것이 인상적이었고, 우주 속 인류의 위치에 대해 깊이 생각하게 되었습니다. 이공계 학생들에게 강력 추천합니다.',
    likes: 53,
    comments: 9,
    timeAgo: '2일 전',
    category: '기술과학',
    program: '2025-2학기 독서 마라톤',
    programId: 'classic100',
  },
  {
    id: 6,
    type: 'program',
    user: { name: '한소희', avatar: 'https://picsum.photos/seed/avatar06/80/80', department: '사학과', studentId: '2023160006' },
    book: { title: '삼국유사', author: '일연', cover: 'https://picsum.photos/seed/cover-samguk/200/280' },
    rating: 4,
    text: '한국 고전 중에서도 가장 풍부한 이야기를 담고 있는 삼국유사를 정독할 수 있어 좋았습니다. 단군신화부터 다양한 설화까지, 우리 민족의 정신세계를 엿볼 수 있었습니다. 사학 전공 수업에서 배운 내용과 연결지어 읽으니 더 의미 있었습니다.',
    likes: 38,
    comments: 7,
    timeAgo: '2일 전',
    category: '문학',
    program: '2025-2학기 독서 마라톤',
    programId: 'classic100',
  },
  {
    id: 7,
    type: 'program',
    user: { name: '윤태호', avatar: 'https://picsum.photos/seed/avatar07/80/80', department: '경제학과', studentId: '2024170007' },
    book: { title: '국부론', author: '애덤 스미스', cover: 'https://picsum.photos/seed/cover-wealth-nations/200/280' },
    rating: 4,
    text: '경제학의 아버지 애덤 스미스의 대표작을 드디어 완독했습니다. 보이지 않는 손의 원리뿐 아니라 분업, 자본 축적, 자유무역에 대한 그의 체계적인 논증이 인상적이었습니다. 현대 경제학 이론의 뿌리를 확인할 수 있는 소중한 경험이었습니다.',
    likes: 47,
    comments: 14,
    timeAgo: '3일 전',
    category: '사회과학',
    program: '2025-2학기 북 멘토링',
    programId: 'bundok',
  },
  {
    id: 8,
    type: 'program',
    user: { name: '강하은', avatar: 'https://picsum.photos/seed/avatar08/80/80', department: '영어영문학과', studentId: '2023180008' },
    book: { title: '오만과 편견', author: '제인 오스틴', cover: 'https://picsum.photos/seed/cover-pride/200/280' },
    rating: 5,
    text: '원서로 읽으면서 오스틴의 위트와 아이러니를 더 깊이 느낄 수 있었습니다. 엘리자베스와 다아시의 관계가 단순한 로맨스가 아니라 계급, 편견, 자아에 대한 깊은 성찰을 담고 있다는 점을 토론을 통해 재발견했습니다.',
    likes: 61,
    comments: 20,
    timeAgo: '3일 전',
    category: '문학',
    program: '2025-2학기 독서 마라톤',
    programId: 'classic100',
  },
  {
    id: 9,
    type: 'program',
    user: { name: '서진우', avatar: 'https://picsum.photos/seed/avatar09/80/80', department: '물리학과', studentId: '2024190009' },
    book: { title: '시간의 역사', author: '스티븐 호킹', cover: 'https://picsum.photos/seed/cover-brief-history/200/280' },
    rating: 4,
    text: '빅뱅부터 블랙홀까지, 우주의 시간에 대한 호킹의 설명은 명료하면서도 깊이가 있었습니다. 물리학 전공자로서 수식 없이 이렇게 명확하게 설명할 수 있다는 것 자체가 놀라웠습니다. 양자역학과 상대성이론의 통합에 대한 그의 비전이 인상적입니다.',
    likes: 34,
    comments: 8,
    timeAgo: '4일 전',
    category: '기술과학',
    program: '2025-2학기 독서 마라톤',
    programId: 'classic100',
  },
  {
    id: 10,
    type: 'program',
    user: { name: '임채원', avatar: 'https://picsum.photos/seed/avatar10/80/80', department: '미술학과', studentId: '2023200010' },
    book: { title: '서양미술사', author: 'E.H. 곰브리치', cover: 'https://picsum.photos/seed/cover-art-history/200/280' },
    rating: 5,
    text: '미술사의 바이블이라 불리는 이 책을 독서프로그램을 통해 체계적으로 읽을 수 있어 감사합니다. 곰브리치의 서술은 단순한 양식사를 넘어 각 시대의 예술가들이 왜 그렇게 표현했는지를 이해하게 해줍니다. 미술을 공부하는 모든 학생에게 필독서입니다.',
    likes: 45,
    comments: 11,
    timeAgo: '5일 전',
    category: '문학',
    program: '2025-2학기 북 멘토링',
    programId: 'bundok',
  },
  {
    id: 11,
    type: 'ogeoseo',
    user: { name: '김도현', avatar: 'https://picsum.photos/seed/avatar11/80/80', department: '소프트웨어학과', studentId: '2024210011' },
    book: { title: '채식주의자', author: '한강', cover: 'https://picsum.photos/seed/cover-vegetarian/200/280' },
    rating: 5,
    text: '한강 작가의 노벨문학상 수상작을 다시 읽었습니다. 영혜의 거부가 단순한 채식이 아니라 폭력적 세계에 대한 저항이라는 점이 다시 한번 가슴에 와닿았습니다. 세 부분으로 나뉜 서사 구조가 주제를 다층적으로 보여주는 방식이 탁월합니다.',
    likes: 78,
    comments: 25,
    timeAgo: '6시간 전',
    category: '문학',
    programId: 'free',
  },
  {
    id: 12,
    type: 'ogeoseo',
    user: { name: '오수빈', avatar: 'https://picsum.photos/seed/avatar12/80/80', department: '정치외교학과', studentId: '2023220012' },
    book: { title: '사회계약론', author: '장 자크 루소', cover: 'https://picsum.photos/seed/cover-social-contract/200/280' },
    rating: 4,
    text: '"인간은 자유롭게 태어났으나 어디서나 쇠사슬에 묶여 있다"는 첫 문장부터 강렬합니다. 일반의지(volonté générale)라는 개념이 현대 민주주의에 어떤 영향을 미쳤는지 생각해 보게 되었고, 직접민주주의에 대한 루소의 이상이 오늘날에도 유효한지 고민하게 됩니다.',
    likes: 36,
    comments: 10,
    timeAgo: '1일 전',
    category: '사회과학',
    programId: 'free',
  },
  {
    id: 13,
    type: 'ogeoseo',
    user: { name: '장민재', avatar: 'https://picsum.photos/seed/avatar13/80/80', department: '화학과', studentId: '2024230013' },
    book: { title: '종의 기원', author: '찰스 다윈', cover: 'https://picsum.photos/seed/cover-origin-species/200/280' },
    rating: 4,
    text: '자연선택을 통한 진화론의 핵심을 다윈 자신의 문체로 읽는 것은 특별한 경험이었습니다. 갈라파고스 핀치새의 관찰에서 시작된 거대한 이론 체계가 얼마나 치밀한 논증 위에 세워졌는지 감탄하게 됩니다. 과학 고전 중 반드시 읽어야 할 책입니다.',
    likes: 29,
    comments: 6,
    timeAgo: '3일 전',
    category: '기술과학',
    programId: 'free',
  },
  {
    id: 14,
    type: 'ogeoseo',
    user: { name: '배서연', avatar: 'https://picsum.photos/seed/avatar14/80/80', department: '교육학과', studentId: '2023240014' },
    book: { title: '논어', author: '공자', cover: 'https://picsum.photos/seed/cover-analects/200/280' },
    rating: 5,
    text: '동양 고전의 정수라 할 수 있는 논어를 완독했습니다. 인(仁)과 예(禮)에 대한 공자의 가르침이 2500년이 지난 지금도 울림이 있다는 것이 놀랍습니다. 교육학을 공부하는 학생으로서, "不憤不啓 不悱不發"(분발하지 않으면 깨우쳐 주지 않는다)이라는 교육 철학에 깊이 공감했습니다.',
    likes: 52,
    comments: 16,
    timeAgo: '4일 전',
    category: '문학',
    programId: 'free',
  },
  {
    id: 15,
    type: 'ogeoseo',
    user: { name: '홍지아', avatar: 'https://picsum.photos/seed/avatar15/80/80', department: '의예과', studentId: '2024250015' },
    book: { title: '엔트로피', author: '제레미 리프킨', cover: 'https://picsum.photos/seed/cover-entropy/200/280' },
    rating: 3,
    text: '열역학 제2법칙을 사회과학적으로 확장한 시도는 흥미로웠으나, 일부 논증에서 과학적 개념을 지나치게 확대 해석한 부분이 아쉬웠습니다. 그럼에도 에너지와 환경 문제를 엔트로피의 관점에서 조망하는 독특한 시각은 충분히 읽을 가치가 있습니다.',
    likes: 18,
    comments: 4,
    timeAgo: '5일 전',
    category: '기술과학',
    programId: 'free',
  },
  // 동일 학생이 여러 프로그램에서 서평 작성한 추가 데이터
  {
    id: 16,
    type: 'program',
    user: { name: '김서윤', avatar: 'https://picsum.photos/seed/avatar01/80/80', department: '국어국문학과', studentId: '2023110001' },
    book: { title: '채식주의자', author: '한강', cover: 'https://picsum.photos/seed/cover-vegetarian/200/280' },
    rating: 5,
    text: '한강 작가 특유의 감각적 문체가 독자를 압도합니다. 채식이라는 행위가 단순한 식습관 변화가 아니라 세상에 대한 근본적 거부임을 느꼈습니다. 국문학도로서 서사 구조의 정교함에 감탄했습니다.',
    likes: 45,
    comments: 12,
    timeAgo: '6일 전',
    category: '문학',
    program: '2025-2학기 독서 마라톤',
    programId: 'bundok',
  },
  {
    id: 17,
    type: 'ogeoseo',
    user: { name: '김서윤', avatar: 'https://picsum.photos/seed/avatar01/80/80', department: '국어국문학과', studentId: '2023110001' },
    book: { title: '논어', author: '공자', cover: 'https://picsum.photos/seed/cover-analects/200/280' },
    rating: 4,
    text: '논어를 국문학적 관점에서 읽으니 번역의 문제와 해석의 다양성이 더욱 눈에 들어왔습니다. 고전 번역의 중요성을 실감하게 되는 독서 경험이었습니다.',
    likes: 30,
    comments: 8,
    timeAgo: '8일 전',
    category: '문학',
    programId: 'free',
  },
  {
    id: 18,
    type: 'program',
    user: { name: '이준혁', avatar: 'https://picsum.photos/seed/avatar02/80/80', department: '경영학과', studentId: '2023120002' },
    book: { title: '국부론', author: '애덤 스미스', cover: 'https://picsum.photos/seed/cover-wealth-nations/200/280' },
    rating: 5,
    text: '경영학도로서 반드시 읽어야 할 고전입니다. 분업과 시장의 원리에 대한 스미스의 통찰이 현대 경영에도 그대로 적용된다는 사실에 놀랐습니다.',
    likes: 42,
    comments: 10,
    timeAgo: '7일 전',
    category: '사회과학',
    program: '2025-2학기 독서 마라톤',
    programId: 'classic100',
  },
  {
    id: 19,
    type: 'ogeoseo',
    user: { name: '이준혁', avatar: 'https://picsum.photos/seed/avatar02/80/80', department: '경영학과', studentId: '2023120002' },
    book: { title: '사피엔스', author: '유발 하라리', cover: 'https://picsum.photos/seed/cover-sapiens/200/280' },
    rating: 4,
    text: '인류의 역사를 경영학적 시각으로 재해석하며 읽었습니다. 농업혁명이 과연 진보였는지에 대한 하라리의 도발적 질문이 흥미로웠습니다.',
    likes: 35,
    comments: 9,
    timeAgo: '10일 전',
    category: '사회과학',
    programId: 'free',
  },
  {
    id: 20,
    type: 'program',
    user: { name: '박지민', avatar: 'https://picsum.photos/seed/avatar03/80/80', department: '생명과학과', studentId: '2024130003' },
    book: { title: '종의 기원', author: '찰스 다윈', cover: 'https://picsum.photos/seed/cover-origin-species/200/280' },
    rating: 5,
    text: '진화론의 원전을 생명과학 전공자로서 읽으니 감회가 남다릅니다. 다윈의 관찰력과 논증 방식은 현대 과학자에게도 귀감이 됩니다.',
    likes: 50,
    comments: 11,
    timeAgo: '9일 전',
    category: '기술과학',
    program: '2025-2학기 북 멘토링',
    programId: 'bundok',
  },
  {
    id: 21,
    type: 'ogeoseo',
    user: { name: '박지민', avatar: 'https://picsum.photos/seed/avatar03/80/80', department: '생명과학과', studentId: '2024130003' },
    book: { title: '코스모스', author: '칼 세이건', cover: 'https://picsum.photos/seed/cover-cosmos/200/280' },
    rating: 4,
    text: '생명과학을 넘어 우주라는 더 큰 맥락에서 생명의 의미를 생각하게 된 책입니다. 세이건의 서술이 과학적이면서도 시적이어서 읽는 내내 몰입했습니다.',
    likes: 28,
    comments: 5,
    timeAgo: '12일 전',
    category: '기술과학',
    programId: 'free',
  },
  {
    id: 22,
    type: 'program',
    user: { name: '최예린', avatar: 'https://picsum.photos/seed/avatar04/80/80', department: '철학과', studentId: '2023140004' },
    book: { title: '정의란 무엇인가', author: '마이클 샌델', cover: 'https://picsum.photos/seed/cover-justice/200/280' },
    rating: 5,
    text: '철학과 학생으로서 샌델의 정의론 강의를 책으로 다시 접하니 새로운 관점이 열렸습니다. 공동체주의적 관점에서 바라본 정의의 개념이 특히 공감되었습니다.',
    likes: 55,
    comments: 17,
    timeAgo: '11일 전',
    category: '사회과학',
    program: '2025-2학기 독서 마라톤',
    programId: 'classic100',
  },
  {
    id: 23,
    type: 'ogeoseo',
    user: { name: '정민수', avatar: 'https://picsum.photos/seed/avatar05/80/80', department: '컴퓨터공학과', studentId: '2024150005' },
    book: { title: '시간의 역사', author: '스티븐 호킹', cover: 'https://picsum.photos/seed/cover-brief-history/200/280' },
    rating: 4,
    text: '프로그래밍과 물리학의 접점을 찾으며 읽었습니다. 시뮬레이션 우주론에 대한 호킹의 입장이 컴퓨터과학도로서 특히 흥미로웠습니다.',
    likes: 22,
    comments: 6,
    timeAgo: '13일 전',
    category: '기술과학',
    programId: 'free',
  },
  {
    id: 24,
    type: 'program',
    user: { name: '한소희', avatar: 'https://picsum.photos/seed/avatar06/80/80', department: '사학과', studentId: '2023160006' },
    book: { title: '역사란 무엇인가', author: 'E.H. 카', cover: 'https://picsum.photos/seed/cover-what-is-history/200/280' },
    rating: 5,
    text: '사학도에게 이 책은 필독서입니다. 역사와 역사가의 관계에 대한 카의 명쾌한 분석이 역사 연구의 방법론을 다시 생각하게 만들었습니다.',
    likes: 40,
    comments: 13,
    timeAgo: '14일 전',
    category: '사회과학',
    program: '2025-2학기 북 멘토링',
    programId: 'bundok',
  },
  {
    id: 25,
    type: 'ogeoseo',
    user: { name: '김도현', avatar: 'https://picsum.photos/seed/avatar11/80/80', department: '소프트웨어학과', studentId: '2024210011' },
    book: { title: '코스모스', author: '칼 세이건', cover: 'https://picsum.photos/seed/cover-cosmos/200/280' },
    rating: 5,
    text: '소프트웨어 개발자의 시선으로 우주를 바라보니, 복잡한 시스템의 질서와 아름다움이 코드 구조와 닮아 있다는 생각이 들었습니다. 세이건의 글은 과학을 넘어선 인문학입니다.',
    likes: 33,
    comments: 7,
    timeAgo: '15일 전',
    category: '기술과학',
    programId: 'free',
  },
]

export const classics: Classic100[] = [
  // 문학 (10)
  {
    id: 1,
    title: '논어',
    author: '공자',
    publisher: '을유문화사',
    year: 2020,
    category: '문학',
    description: '동양 철학의 근간이 되는 고전으로, 공자와 그 제자들의 대화를 기록한 책입니다. 인(仁), 의(義), 예(禮), 지(智) 등 유교의 핵심 덕목을 배울 수 있으며, 오늘날에도 인간관계와 자기 수양에 대한 깊은 통찰을 제공합니다.',
    cover: 'https://images.unsplash.com/photo-1544947950-fa07a98d237f?w=200&h=280&fit=crop',
    isbn: '978-89-324-7380-5',
  },
  {
    id: 2,
    title: '시경',
    author: '작자 미상 (공자 편집)',
    publisher: '민음사',
    year: 2018,
    category: '문학',
    description: '중국 최초의 시가 총집으로, 주나라 초기부터 춘추시대까지의 시 305편을 수록하고 있습니다. 풍(風)·아(雅)·송(頌)으로 분류되며, 당시 민중의 삶과 감정, 사회상을 생생하게 보여주는 동양 문학의 원류입니다.',
    cover: 'https://images.unsplash.com/photo-1512820790803-83ca734da794?w=200&h=280&fit=crop',
    isbn: '978-89-374-3456-7',
  },
  {
    id: 3,
    title: '채식주의자',
    author: '한강',
    publisher: '창비',
    year: 2022,
    category: '문학',
    description: '2016년 맨부커 국제상, 2024년 노벨문학상을 수상한 한강의 대표작입니다. 고기를 거부하는 한 여성의 이야기를 통해 폭력, 욕망, 인간 존재의 근원적 문제를 탐구합니다. 세 편의 연작 소설로 구성된 독특한 서사 구조가 특징입니다.',
    cover: 'https://images.unsplash.com/photo-1543002588-bfa74002ed7e?w=200&h=280&fit=crop',
    isbn: '978-89-364-7234-1',
  },
  {
    id: 4,
    title: '서양미술사',
    author: 'E.H. 곰브리치',
    publisher: '예경',
    year: 2017,
    category: '문학',
    description: '미술사 입문서의 바이블로 불리는 곰브리치의 대표작입니다. 고대 이집트부터 현대 미술까지 서양 미술의 흐름을 일관된 시각으로 서술하며, 왜 예술가들이 특정 방식으로 표현했는지를 이해하도록 돕습니다.',
    cover: 'https://images.unsplash.com/photo-1476275466078-4007374efbbe?w=200&h=280&fit=crop',
    isbn: '978-89-7084-456-2',
  },
  {
    id: 5,
    title: '삼국유사',
    author: '일연',
    publisher: '한길사',
    year: 2019,
    category: '문학',
    description: '고려 시대 승려 일연이 편찬한 역사서로, 한국 고대사와 설화, 불교 문화를 담고 있습니다. 단군신화를 비롯한 건국 설화와 다양한 민간 전승이 수록되어 있어, 한국 문화의 원형을 이해하는 데 필수적인 고전입니다.',
    cover: 'https://images.unsplash.com/photo-1589998059171-988d887df646?w=200&h=280&fit=crop',
    isbn: '978-89-356-1234-5',
  },
  {
    id: 6,
    title: '오만과 편견',
    author: '제인 오스틴',
    publisher: '펭귄클래식코리아',
    year: 2021,
    category: '문학',
    description: '19세기 영국 사회를 배경으로 엘리자베스 베넷과 피츠윌리엄 다아시의 사랑 이야기를 그린 소설입니다. 위트 있는 대화와 섬세한 인물 묘사를 통해 계급, 결혼, 편견에 대한 날카로운 사회 비평을 담고 있습니다.',
    cover: 'https://images.unsplash.com/photo-1497633762265-9d179a990aa6?w=200&h=280&fit=crop',
    isbn: '978-89-01-1234-5',
  },
  {
    id: 7,
    title: '데미안',
    author: '헤르만 헤세',
    publisher: '민음사',
    year: 2020,
    category: '문학',
    description: '소년 싱클레어의 자아 탐색과 성장을 그린 헤세의 대표적 성장소설입니다. 선과 악, 밝음과 어둠 사이에서 진정한 자아를 찾아가는 과정을 그리며, 청년기의 방황과 각성에 대한 보편적 메시지를 전달합니다.',
    cover: 'https://images.unsplash.com/photo-1495446815901-a7297e633e8d?w=200&h=280&fit=crop',
    isbn: '978-89-374-6012-3',
  },
  {
    id: 8,
    title: '햄릿',
    author: '윌리엄 셰익스피어',
    publisher: '열린책들',
    year: 2016,
    category: '문학',
    description: '셰익스피어 4대 비극 중 하나로, 덴마크 왕자 햄릿의 복수극을 다룹니다. "사느냐 죽느냐 그것이 문제로다"라는 유명한 독백에서 드러나듯, 인간 존재의 근본적 질문과 결단의 문제를 깊이 탐구합니다.',
    cover: 'https://images.unsplash.com/photo-1519682337058-a94d519337bc?w=200&h=280&fit=crop',
    isbn: '978-89-329-1567-8',
  },
  {
    id: 9,
    title: '변신',
    author: '프란츠 카프카',
    publisher: '문학동네',
    year: 2019,
    category: '문학',
    description: '어느 날 아침 벌레로 변한 그레고르 잠자의 이야기를 통해 인간 소외와 부조리를 그린 카프카의 대표 중편소설입니다. 현대 사회에서 개인이 겪는 정체성의 위기와 가족 관계의 본질을 날카롭게 포착합니다.',
    cover: 'https://images.unsplash.com/photo-1550399105-c4db5fb85c18?w=200&h=280&fit=crop',
    isbn: '978-89-546-3456-7',
  },
  {
    id: 10,
    title: '목민심서',
    author: '정약용',
    publisher: '창비',
    year: 2018,
    category: '문학',
    description: '조선 후기 실학자 정약용이 저술한 목민관의 지침서입니다. 관리가 갖추어야 할 덕목과 백성을 위한 정치의 이상을 체계적으로 제시하며, 공직 윤리와 행정의 원칙에 대한 시대를 초월한 가르침을 담고 있습니다.',
    cover: 'https://images.unsplash.com/photo-1524578271613-d550eacf6090?w=200&h=280&fit=crop',
    isbn: '978-89-364-8901-2',
  },
  // 사회과학 (10)
  {
    id: 11,
    title: '국부론',
    author: '애덤 스미스',
    publisher: '비봉출판사',
    year: 2017,
    category: '사회과학',
    description: '경제학의 아버지 애덤 스미스의 대표작으로, 자유시장 경제의 이론적 토대를 확립한 고전입니다. 분업, 자본 축적, 자유무역의 원리를 체계적으로 분석하며, "보이지 않는 손"의 개념으로 시장경제의 자기 조절 메커니즘을 설명합니다.',
    cover: 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=200&h=280&fit=crop',
    isbn: '978-89-376-0456-7',
  },
  {
    id: 12,
    title: '정의론',
    author: '존 롤스',
    publisher: '이학사',
    year: 2019,
    category: '사회과학',
    description: '20세기 가장 영향력 있는 정치철학서로, "공정으로서의 정의" 개념을 제시합니다. 무지의 베일과 원초적 입장이라는 사고 실험을 통해 사회 정의의 원칙을 도출하며, 자유와 평등의 조화를 모색하는 현대 정치철학의 기념비적 저작입니다.',
    cover: 'https://images.unsplash.com/photo-1516979187457-637abb4f9353?w=200&h=280&fit=crop',
    isbn: '978-89-561-3456-7',
  },
  {
    id: 13,
    title: '자본론',
    author: '카를 마르크스',
    publisher: '비봉출판사',
    year: 2015,
    category: '사회과학',
    description: '자본주의 경제 체제를 분석한 마르크스의 대표작입니다. 상품, 화폐, 자본의 개념부터 잉여가치, 자본 축적, 공황에 이르기까지 자본주의의 운동 법칙을 체계적으로 해부하며, 현대 사회과학에 지대한 영향을 미쳤습니다.',
    cover: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200&h=280&fit=crop',
    isbn: '978-89-376-0789-1',
  },
  {
    id: 14,
    title: '사회계약론',
    author: '장 자크 루소',
    publisher: '펭귄클래식코리아',
    year: 2018,
    category: '사회과학',
    description: '프랑스 혁명에 사상적 기초를 제공한 루소의 정치철학서입니다. 인민주권론과 일반의지(volonté générale)의 개념을 통해 정당한 정치 권력의 근거를 탐구하며, 자유와 평등을 실현하는 민주적 공동체의 이상을 제시합니다.',
    cover: 'https://images.unsplash.com/photo-1532012197267-da84d127e765?w=200&h=280&fit=crop',
    isbn: '978-89-01-2345-6',
  },
  {
    id: 15,
    title: '존재와 시간',
    author: '마르틴 하이데거',
    publisher: '까치',
    year: 2016,
    category: '사회과학',
    description: '20세기 존재론의 전환점이 된 하이데거의 주저입니다. 현존재(Dasein)의 존재 구조를 분석하며, 세계-내-존재, 불안, 죽음에 대한 깊은 철학적 성찰을 전개합니다. 실존주의와 해석학에 결정적인 영향을 미친 현대 철학의 기념비적 저작입니다.',
    cover: 'https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=200&h=280&fit=crop',
    isbn: '978-89-7291-456-7',
  },
  {
    id: 16,
    title: '정의란 무엇인가',
    author: '마이클 샌델',
    publisher: '와이즈베리',
    year: 2023,
    category: '사회과학',
    description: '하버드대 마이클 샌델 교수의 정치철학 강의를 담은 책으로, 정의에 대한 다양한 철학적 접근을 탐구합니다. 공리주의, 자유주의, 공동체주의 등의 관점에서 현실 사례를 분석하며 독자 스스로 정의에 대해 사유하도록 이끕니다.',
    cover: 'https://images.unsplash.com/photo-1491841550275-ad7854e35ca6?w=200&h=280&fit=crop',
    isbn: '978-89-6217-123-4',
  },
  {
    id: 17,
    title: '군주론',
    author: '니콜로 마키아벨리',
    publisher: '까치',
    year: 2020,
    category: '사회과학',
    description: '르네상스 시대 피렌체의 정치사상가 마키아벨리의 대표작으로, 정치 권력의 획득과 유지에 관한 현실주의적 분석을 담고 있습니다. 도덕과 정치의 관계에 대한 파격적 주장은 근대 정치학의 출발점이 되었습니다.',
    cover: 'https://images.unsplash.com/photo-1513001900722-370f803f498d?w=200&h=280&fit=crop',
    isbn: '978-89-7291-789-2',
  },
  {
    id: 18,
    title: '역사란 무엇인가',
    author: 'E.H. 카',
    publisher: '까치',
    year: 2021,
    category: '사회과학',
    description: '역사학의 본질과 방법론을 탐구한 E.H. 카의 명저입니다. "역사는 현재와 과거의 끊임없는 대화"라는 유명한 정의를 통해 역사적 사실과 해석의 관계, 역사가의 역할에 대한 근본적 성찰을 제시합니다.',
    cover: 'https://images.unsplash.com/photo-1457369804613-52c61a468e7d?w=200&h=280&fit=crop',
    isbn: '978-89-7291-234-5',
  },
  {
    id: 19,
    title: '프로테스탄트 윤리와 자본주의 정신',
    author: '막스 베버',
    publisher: '문예출판사',
    year: 2019,
    category: '사회과학',
    description: '종교와 경제의 관계를 분석한 사회학의 고전입니다. 프로테스탄티즘의 금욕적 윤리가 근대 자본주의의 발전에 기여했다는 독창적 테제를 제시하며, 문화와 경제 발전의 관계에 대한 새로운 시각을 열어주었습니다.',
    cover: 'https://images.unsplash.com/photo-1509021436665-8f07dbf5bf1d?w=200&h=280&fit=crop',
    isbn: '978-89-310-0456-7',
  },
  {
    id: 20,
    title: '유토피아',
    author: '토머스 모어',
    publisher: '서해문집',
    year: 2017,
    category: '사회과학',
    description: '16세기 영국의 사상가 토머스 모어가 이상적 사회를 상상하여 그린 작품입니다. 사유재산이 없는 평등한 공동체를 묘사하며, 현실 사회의 부조리를 풍자하고 더 나은 사회에 대한 사유를 촉구합니다.',
    cover: 'https://images.unsplash.com/photo-1515125520141-3e3b67bc0a88?w=200&h=280&fit=crop',
    isbn: '978-89-7483-456-7',
  },
  // 기술과학 (10)
  {
    id: 21,
    title: '종의 기원',
    author: '찰스 다윈',
    publisher: '사이언스북스',
    year: 2019,
    category: '기술과학',
    description: '자연선택에 의한 진화론을 체계적으로 제시한 생물학의 기념비적 저작입니다. 다윈은 방대한 관찰과 실험 자료를 바탕으로 생물 종의 변이와 적응, 자연선택의 메커니즘을 논증하며 생명과학의 패러다임을 근본적으로 전환시켰습니다.',
    cover: 'https://images.unsplash.com/photo-1474366521946-c3d4b507abf2?w=200&h=280&fit=crop',
    isbn: '978-89-8371-456-7',
  },
  {
    id: 22,
    title: '코스모스',
    author: '칼 세이건',
    publisher: '사이언스북스',
    year: 2022,
    category: '기술과학',
    description: '천체물리학자 칼 세이건이 우주와 인류의 이야기를 담은 과학 교양서의 걸작입니다. 우주의 탄생부터 외계 생명의 가능성까지 과학적 사실을 시적이고 감동적인 문체로 서술하며, 우주 속 인류의 위치와 의미를 성찰합니다.',
    cover: 'https://images.unsplash.com/photo-1462759353907-b2ea5ebd72e7?w=200&h=280&fit=crop',
    isbn: '978-89-8371-789-1',
  },
  {
    id: 23,
    title: '이기적 유전자',
    author: '리처드 도킨스',
    publisher: '을유문화사',
    year: 2023,
    category: '기술과학',
    description: '유전자의 관점에서 생명 현상을 재해석한 혁명적 저작입니다. 이타적 행동을 포함한 모든 생명 현상을 유전자의 자기 복제 전략으로 설명하며, 문화적 유전 단위로서의 밈(meme) 개념을 최초로 제안한 것으로도 유명합니다.',
    cover: 'https://images.unsplash.com/photo-1535905557558-afc4877a26fc?w=200&h=280&fit=crop',
    isbn: '978-89-324-7456-7',
  },
  {
    id: 24,
    title: '시간의 역사',
    author: '스티븐 호킹',
    publisher: '까치',
    year: 2021,
    category: '기술과학',
    description: '빅뱅부터 블랙홀, 시간의 본질까지 현대 물리학의 핵심 주제를 일반인도 이해할 수 있도록 풀어쓴 과학 교양서입니다. 우주의 기원과 구조, 양자역학과 상대성이론의 통합이라는 물리학의 궁극적 과제를 명쾌하게 소개합니다.',
    cover: 'https://images.unsplash.com/photo-1506880018603-83d5b814b5a6?w=200&h=280&fit=crop',
    isbn: '978-89-7291-567-8',
  },
  {
    id: 25,
    title: '엔트로피',
    author: '제레미 리프킨',
    publisher: '세종서적',
    year: 2020,
    category: '기술과학',
    description: '열역학 제2법칙인 엔트로피 법칙을 사회, 경제, 환경 문제에 적용한 독창적인 저작입니다. 에너지의 불가역적 소산이라는 물리적 원리를 통해 현대 산업문명의 위기를 진단하고 지속 가능한 사회로의 전환을 촉구합니다.',
    cover: 'https://images.unsplash.com/photo-1512436991641-6745cdb1723f?w=200&h=280&fit=crop',
    isbn: '978-89-8407-456-7',
  },
  {
    id: 26,
    title: '침묵의 봄',
    author: '레이첼 카슨',
    publisher: '에코리브르',
    year: 2018,
    category: '기술과학',
    description: '살충제의 무분별한 사용이 생태계에 미치는 영향을 고발한 환경 고전입니다. DDT 등 화학 살충제가 먹이 사슬을 통해 축적되어 조류와 생태계를 파괴하는 과정을 과학적으로 입증하며, 현대 환경 운동의 출발점이 된 기념비적 저작입니다.',
    cover: 'https://images.unsplash.com/photo-1516979187457-637abb4f9353?w=200&h=280&fit=crop',
    isbn: '978-89-6263-123-4',
  },
  {
    id: 27,
    title: '부분과 전체',
    author: '베르너 하이젠베르크',
    publisher: '서커스',
    year: 2016,
    category: '기술과학',
    description: '양자역학의 창시자 중 한 명인 하이젠베르크의 자서전적 과학서입니다. 20세기 물리학의 혁명적 발견들이 어떤 대화와 논쟁을 통해 이루어졌는지를 생생하게 기록하며, 과학과 철학, 예술의 관계를 깊이 성찰합니다.',
    cover: 'https://images.unsplash.com/photo-1497633762265-9d179a990aa6?w=200&h=280&fit=crop',
    isbn: '978-89-6578-123-4',
  },
  {
    id: 28,
    title: '통섭',
    author: '에드워드 윌슨',
    publisher: '사이언스북스',
    year: 2020,
    category: '기술과학',
    description: '자연과학과 인문학의 통합을 주장한 사회생물학자 에드워드 윌슨의 대표작입니다. 물리학, 화학, 생물학, 사회과학, 인문학의 경계를 넘어 지식의 대통합을 제안하며, 학문 간 융합의 필요성과 가능성을 역설합니다.',
    cover: 'https://images.unsplash.com/photo-1495446815901-a7297e633e8d?w=200&h=280&fit=crop',
    isbn: '978-89-8371-234-5',
  },
  {
    id: 29,
    title: '숨결이 바람 될 때',
    author: '폴 칼라니티',
    publisher: '흐름출판',
    year: 2023,
    category: '기술과학',
    description: '신경외과 레지던트였던 저자가 폐암 진단을 받은 후 삶과 죽음, 의학의 의미를 성찰한 자전적 에세이입니다. 의사이자 환자로서의 이중적 시선을 통해 인간 존재의 유한성과 그 속에서 의미를 찾는 과정을 감동적으로 그려냅니다.',
    cover: 'https://images.unsplash.com/photo-1519682337058-a94d519337bc?w=200&h=280&fit=crop',
    isbn: '978-89-6596-234-5',
  },
  {
    id: 30,
    title: '파인만의 물리학 강의',
    author: '리처드 파인만',
    publisher: '승산',
    year: 2017,
    category: '기술과학',
    description: '노벨물리학상 수상자 리처드 파인만의 전설적인 물리학 입문 강의를 정리한 책입니다. 역학, 전자기학, 양자역학의 핵심 개념을 파인만 특유의 직관적이고 명쾌한 방식으로 설명하며, 물리학의 아름다움과 재미를 전달합니다.',
    cover: 'https://images.unsplash.com/photo-1550399105-c4db5fb85c18?w=200&h=280&fit=crop',
    isbn: '978-89-6139-123-4',
  },
]

export const bundoks: Bundok[] = [
  {
    id: 1,
    title: "카페에서 읽는 데미안",
    book: "데미안",
    bookAuthor: "헤르만 헤세",
    bookCover: "https://images.unsplash.com/photo-1495446815901-a7297e633e8d?w=200&h=280&fit=crop",
    host: "김서윤",
    hostNickname: "책벌레서윤",
    hostAvatar: "https://picsum.photos/seed/host1/80/80",
    format: "offline",
    location: "스터디룸 2-1",
    date: "2026-02-25",
    time: "19:00",
    duration: 90,
    currentMembers: 3,
    maxMembers: 5,
    tags: ["문학", "성장소설", "고전"],
    status: "recruiting",
    members: [
      { nickname: "책벌레윤", realName: "윤서현" },
      { nickname: "독서왕", realName: "이도윤" },
      { nickname: "문학소녀", realName: "박하은" },
    ],
    aiBooks: ["싯다르타", "수레바퀴 아래서", "나르치스와 골드문트"],
    discussionQuestions: [
      "싱클레어에게 데미안은 어떤 의미인가?",
      "아브락사스 상징이 현대 사회에서 의미하는 바는?",
      "'새는 알에서 나오려고 투쟁한다'는 문장을 자신의 경험에 비춰 해석해보세요.",
    ],
  },
  {
    id: 2,
    title: "점심시간 사피엔스 토론",
    book: "사피엔스",
    bookAuthor: "유발 하라리",
    bookCover: "https://images.unsplash.com/photo-1497633762265-9d179a990aa6?w=200&h=280&fit=crop",
    host: "이준혁",
    hostNickname: "역사덕후",
    hostAvatar: "https://picsum.photos/seed/host2/80/80",
    format: "offline",
    location: "스터디룸 3-1",
    date: "2026-02-27",
    time: "12:00",
    duration: 60,
    currentMembers: 4,
    maxMembers: 5,
    tags: ["인문", "역사", "과학"],
    status: "recruiting",
    members: [
      { nickname: "역사탐험가", realName: "김태현" },
      { nickname: "하라리팬", realName: "정수빈" },
      { nickname: "사피엔스", realName: "오지훈" },
      { nickname: "인문학도", realName: "강예진" },
    ],
    aiBooks: ["호모 데우스", "21세기를 위한 21가지 제언", "총, 균, 쇠"],
    discussionQuestions: [
      "농업혁명이 인류에게 축복이었을까, 재앙이었을까?",
      "허구를 믿는 능력이 인간 사회에 미친 영향은?",
    ],
  },
  {
    id: 3,
    title: "온라인 철학 읽기 모임",
    book: "정의란 무엇인가",
    bookAuthor: "마이클 샌델",
    bookCover: "https://images.unsplash.com/photo-1491841550275-ad7854e35ca6?w=200&h=280&fit=crop",
    host: "박지민",
    hostNickname: "철학하는지민",
    hostAvatar: "https://picsum.photos/seed/host3/80/80",
    format: "online",
    location: "Zoom",
    date: "2026-03-01",
    time: "20:00",
    duration: 120,
    currentMembers: 2,
    maxMembers: 4,
    tags: ["철학", "사회과학", "토론"],
    status: "recruiting",
    members: [
      { nickname: "철학하는곰", realName: "송민재" },
      { nickname: "생각하는사람", realName: "임채원" },
    ],
    aiBooks: ["공정하다는 착각", "왜 도덕인가", "자유론"],
    discussionQuestions: [
      "트롤리 딜레마에서 당신의 선택은?",
      "공리주의와 자유지상주의의 한계는 무엇인가?",
    ],
  },
  {
    id: 4,
    title: "채식주의자 깊이 읽기",
    book: "채식주의자",
    bookAuthor: "한강",
    bookCover: "https://images.unsplash.com/photo-1543002588-bfa74002ed7e?w=200&h=280&fit=crop",
    host: "최예린",
    hostNickname: "한강러버",
    hostAvatar: "https://picsum.photos/seed/host4/80/80",
    format: "hybrid",
    location: "스터디룸 4-1 / Zoom",
    date: "2026-02-22",
    time: "18:00",
    duration: 90,
    currentMembers: 5,
    maxMembers: 5,
    tags: ["문학", "한국문학", "노벨문학상"],
    status: "confirmed",
    members: [
      { nickname: "한강러버", realName: "최예린" },
      { nickname: "소설읽는밤", realName: "김나윤" },
      { nickname: "문학청년", realName: "이서준" },
      { nickname: "글쓰는고양이", realName: "박소율" },
      { nickname: "북마크", realName: "장하린" },
    ],
    aiBooks: ["소년이 온다", "흰", "작별하지 않는다"],
    discussionQuestions: [
      "영혜의 채식 거부는 어떤 상징인가?",
      "세 편의 서사를 연결하는 공통 주제는?",
    ],
  },
  {
    id: 5,
    title: "코스모스와 함께하는 밤",
    book: "코스모스",
    bookAuthor: "칼 세이건",
    bookCover: "https://images.unsplash.com/photo-1462759353907-b2ea5ebd72e7?w=200&h=280&fit=crop",
    host: "정민수",
    hostNickname: "별보는민수",
    hostAvatar: "https://picsum.photos/seed/host5/80/80",
    format: "offline",
    location: "스터디룸 2-1",
    date: "2026-02-20",
    time: "21:00",
    duration: 90,
    currentMembers: 4,
    maxMembers: 4,
    tags: ["과학", "천문학", "교양"],
    status: "confirmed",
    members: [
      { nickname: "별보는소년", realName: "정민수" },
      { nickname: "우주덕후", realName: "한지우" },
      { nickname: "밤하늘", realName: "류서진" },
      { nickname: "과학소녀", realName: "백다은" },
    ],
  },
  {
    id: 6,
    title: "이기적 유전자 완독 챌린지",
    book: "이기적 유전자",
    bookAuthor: "리처드 도킨스",
    bookCover: "https://images.unsplash.com/photo-1535905557558-afc4877a26fc?w=200&h=280&fit=crop",
    host: "한소희",
    hostNickname: "유전자탐험가",
    hostAvatar: "https://picsum.photos/seed/host6/80/80",
    format: "offline",
    location: "스터디룸 3-1",
    date: "2026-02-15",
    time: "14:00",
    duration: 120,
    currentMembers: 3,
    maxMembers: 3,
    tags: ["과학", "생물학", "진화론"],
    status: "completed",
    members: [
      { nickname: "진화론자", realName: "한소희" },
      { nickname: "유전자맨", realName: "조현우" },
      { nickname: "생물학개론", realName: "신예은" },
    ],
  },
  {
    id: 7,
    title: "국부론 함께 읽기",
    book: "국부론",
    bookAuthor: "애덤 스미스",
    bookCover: "https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=200&h=280&fit=crop",
    host: "윤태호",
    hostNickname: "경제학입문",
    hostAvatar: "https://picsum.photos/seed/host7/80/80",
    format: "online",
    location: "Google Meet",
    date: "2026-02-10",
    time: "19:00",
    duration: 90,
    currentMembers: 5,
    maxMembers: 5,
    tags: ["경제", "사회과학", "고전"],
    status: "completed",
    members: [
      { nickname: "경제학도", realName: "윤태호" },
      { nickname: "애덤의손", realName: "김하준" },
      { nickname: "자본론", realName: "이수아" },
      { nickname: "경영학개론", realName: "박지환" },
      { nickname: "주식왕", realName: "최서연" },
    ],
  },
]
